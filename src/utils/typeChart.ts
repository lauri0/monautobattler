...
// Effectiveness multipliers: chart[attackType][defendType]
// 0 = immune, 0.5 = not very effective, 1 = normal, 2 = super effective
const chart: Record<TypeName, Record<TypeName, number>> = {
...
export function getDefensiveMatchups(pokemonTypes: TypeName[]): {
  weaknesses: { type: TypeName; mult: number }[];
  resistances: { type: TypeName; mult: number }[];
  immunities: TypeName[];
} {
  const weaknesses: { type: TypeName; mult: number }[] = [];
  const resistances: { type: TypeName; mult: number }[] = [];
  ...
  for (const attackType of ALL_TYPES) {
    ...
    else if (mult > 1) weaknesses.push({ type: attackType, mult });
    else if (mult < 1) resistances.push({ type: attackType, mult });
  }

  return { weaknesses, resistances, immunities };
}

export function getDetailedDefensiveMatchups(
